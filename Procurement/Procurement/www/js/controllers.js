...
.controller('GetLeadStageAndCount', function($scope, $http, $state, $rootScope, $cordovaCalendar) {
  $scope.data = {
    isLoading: false
   };
  ...
  $scope.GetLeadStageAndCount = function(){
    ...
    method: 'POST',
    dataType: 'json',
    url: 'http://192.168.51.251/lead/api/TELeadAPI/GetLeadStageAndCount',
    headers: {'Content-Type': 'application/json; charset=UTF-8'},
    data: json
    }).success (function(response){
        $scope.names = response.result;
        $scope.Assigned = response.result.Assigned;
        $scope.Connected = response.result.Connected;
        $scope.Enquiry = response.result.Enquiry;
        $scope.Suspect = response.result.Suspect;
  ...
  $scope.GetAllLeadData = function(){
    ...
    data: json
     ...
     $scope.UpcomingFolloups = function(){   
        var json = {FromDate: $scope.TodaysDate,ToDate: $scope.SevenDays};
         //console.log("Today: "+$scope.TodaysDate+"Seven Days :"+$scope.SevenDays);
         $http({
         method: 'POST',
         dataType: 'json',
         url: 'http://192.168.51.251/portfolio/api/TEMobileCallAPI/GetLeadUpComingFollowups',
         headers: {'Content-Type': 'application/json; charset=UTF-8'},
         data: json
         }).success (function(response){
         if(response.result.length > 0){
         $scope.followups = response.result;
         //console.log("followups :"+$scope.followups);
         $scope.LeadID = response.result.LeadID;
         $scope.Date = response.result[0].Date;
         var values =  $scope.Date.split("T");
         $scope.DateFinal = values[0];
         }else{
           console.log("No UP COMMING FOLLOW UPS Data Found!");
         }
       });
     }
     ...
     //Getting Lead Data for pipeline
     $scope.GetAllLeadForPipelineForToady = function(){   
         var json = {FromDate: "2016-10-04", ToDate:"2016-10-04"};
         $http({
         method: 'POST',
         dataType: 'json',
         url: 'http://182.18.177.27/portfolio/api/TEMobileCallAPI/GetLeadsDataForPipeLine',
         headers: {'Content-Type': 'application/json; charset=UTF-8'},
         data: json
         }).success (function(response){
         if(response.result.length > 0){
         $scope.AllPipeline = response.result;
         //console.log("AllPipeline :"+$scope.AllPipeline);
         }else{
           console.log("No Data Found!");
         }
       });
      ...
      //Getting Lead Data for pipeline
     $scope.GetAllLeadForPipelineForSevenDays = function(){   
         var json = {FromDate: $scope.TodaysDate,ToDate: $scope.SevenDays};
         $http({
         method: 'POST',
         dataType: 'json',
         ...
         if(response.result.length > 0){
         $scope.PipelineForSevenDays = response.result;
        // console.log("AllPipelinesFor Seven :"+$scope.AllPipeline);
         }else{
           console.log("No Data Found!");
         }
       });
      ...
      //Getting Lead Data for pipeline
     $scope.UpcommingOfferExpire = function(){   
         var json = {FromDate: "2016-10-04", ToDate:"2016-10-12"};
         $http({
         method: 'POST',
         dataType: 'json',
         url: 'http://182.18.177.27/portfolio/api/TEMobileCallAPI/GetOfferUpComingFollowups',
         headers: {'Content-Type': 'application/json; charset=UTF-8'},
         data: json
         }).success (function(response){
         if(response.result.length > 0){
         $scope.OfferExpire = response.result;
         $scope.ExpiryDate = response.result[0].ExpiryDate;
         var values =  $scope.ExpiryDate.split("T");
         $scope.DateFinal = values[0];
         console.log("OfferExpire:"+  $scope.DateFinal);
         }else{
           console.log("No Data Found!");
         }
       });
  ...
  //get data by lead id
 .controller('GetParticularLeadData', function($scope,$rootScope, $http,  GestOnlyLeadIdByFactory, $stateParams, $state, Auth, $timeout, $window, $location) {
  ...
  var json = {LeadTemperature:name, LeadId:  $scope.LeadID, ContactID: "", LastModifiedBy: "3375", LastModifiedBy_Id:"3375"};
  console.log("LeadTemperature: " +name + "Lead: " +$scope.LeadID);
   $http({
   method: 'POST',
   dataType: 'json',
   url: 'http://192.168.51.248/lead/api/TELeadAPI/UpdateTELeadTemp',
   headers: {'Content-Type': 'application/json; charset=UTF-8'},
   data: json
   }).success (function(response){
     $scope.info = response.info;
     alert($scope.info.errormessage);
     $state.go('app.lead');
     $window.location.reload(true);});
    })
  }
  ...
  $scope.cancelFollowUp = function(){
    ...
    method: 'POST',
    dataType: 'json',
    url: 'http://192.168.51.248/lead/api/TELeadAPI/GetByTELeadIDWithData',
    headers: {'Content-Type': 'application/json; charset=UTF-8'},
    data: json
}).success (function(response){
        $scope.names = response.result;
...
$scope.contactToEdit = {
  ...
  console.log($scope.SalesValue);
    ...
    $scope.countries = [ {name: "United States",dial_code: "+1",code: "US"}, {name: "Israel",dial_code: "+972",code: "IL"}, { name: "Afghanistan", dial_code: "+93",code: "AF"}, {name: "Albania",dial_code: "+355",code: "AL"}, {name: "Algeria",dial_code: "+213",code: "DZ"}, {name: "AmericanSamoa",
    ...
    name: "India",dial_code: "+91",code: "IN"}, {name: "Indonesia",dial_code: "+62", code: "ID"}, {name: "Iraq",dial_code: "+964",code: "IQ"}, {name: "Ireland",dial_code: "+353",code: "IE"}, {name: "Israel",
...
.controller('NewLeadData', function($scope, $http, $filter, $state, $location, $window, $rootScope) {
...
//here we are inserting users lead Temparature(HOT, WARM, COLD, NEW)
.controller('ColdLeadData', function($scope, $http, $filter, $state, $location, $window, $rootScope) {
  var json = {checkonce:"10", pageno:"1", pagepercount:"100000", LeadStageID:"", 
...
//here we are inserting users lead Temparature(HOT, WARM, COLD, NEW)
.controller('WarmLeadData', function($scope, $http, $filter, $state, $location, $window, $rootScope) {
  var json = {checkonce:"10", pageno:"1", pagepercount:"100000", LeadStageID:"", 
...
//here we are inserting users lead Temparature(HOT, WARM, COLD, NEW)
.controller('HotLeadData', function($scope, $http, $filter, $state, $location, $window, $rootScope) {
  var json = {checkonce:"10", pageno:"1", pagepercount:"100000", LeadStageID:"", 
